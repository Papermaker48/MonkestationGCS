...
/client/proc/imaginary_friend()
	...
	var/mob/eye/imaginary_friend/mentor/mentorfriend = new(get_turf(mentee), mentee)
...
/mob/eye/imaginary_friend/mentor/greet()
...
/mob/eye/imaginary_friend/mentor/Login()
	. = ..()
	setup_friend()
	Show()

/mob/eye/imaginary_friend/mentor/Logout()
	. = ..()
	if(!src.key)
		return
	unmentor()

/mob/eye/imaginary_friend/mentor/Initialize(mapload, mob/imaginary_friend_owner, datum/preferences/appearance_from_prefs = null)
	. = ..()
...
/mob/eye/imaginary_friend/mentor/proc/unmentor()
	icon = human_image
...
/mob/eye/imaginary_friend/mentor/recall()
	if(QDELETED(owner))
		unmentor()
		return FALSE
	if(loc == owner)
...
/datum/action/innate/imaginary_leave/Activate()
	var/mob/eye/imaginary_friend/mentor/I = owner
	I.unmentor()
...
/client/proc/create_ifriend(mob/living/friend_owner, seek_confirm = FALSE)
	...
	if(istype(C.mob, /mob/eye/imaginary_friend))
		var/mob/eye/imaginary_friend/IF = C.mob
		IF.ghostize()
		return
	...
	var/mob/eye/imaginary_friend/mentor/mentorfriend = new(get_turf(friend_owner), friend_owner)
